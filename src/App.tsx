...
import { signInWithPassword, getSession as getSupabaseSession, signOut as supabaseSignOut, supabaseConfigured, createAuthAccount, deleteRemoteRecord } from "./lib/supabase";
...
function useGlobalStyles() {
  useEffect(() => {
    ...
    if (existing) existing.remove(); // always rewrite, in case a stale cached version lingers
    ...
    style.textContent = `
      *, *::before, *::after { box-sizing: border-box !important; }
      ...
      @media (max-width: 860px) {
        .pfs-sidebar-hamburger { display: inline-block !important; }
        .pfs-sidebar-close { display: inline-block !important; }
        .pfs-sidebar-backdrop { display: block !important; }
        .pfs-admin-sidebar {
        ...
        .pfs-admin-sidebar-open { transform: translateX(0) !important; }
        .pfs-admin-search { width: 100% !important; max-width: 100% !important; }
        .pfs-admin-topbar { flex-wrap: wrap; gap: 8px; }
        .pfs-kpi-5 { grid-template-columns: repeat(2, minmax(0,1fr)) !important; }
        .pfs-2col-stack { grid-template-columns: minmax(0,1fr) !important; }
      }
    `;
...
async function saveData(data) {
  await storage.set(STORAGE_KEY, JSON.stringify(data)); // let errors propagate — persist() surfaces them to the user
}
...
function GeoMap({ sites, supervisors, activeSiteId, onSelectSite, height = 260, locationRecords = [] }) {
  ...
  return (
    <div style={{ position: "relative", height, borderRadius: 12, overflow: "hidden", border: `1px solid ${COLORS.line}` }}>
      ...
      <div style={{ position: "absolute", top: 8, left: 10, zIndex: 1000, fontSize: 10.5, color: "#fff", textShadow: "0 1px 2px rgba(0,0,0,0.8)", fontFamily: "IBM Plex Mono, monospace", display: "flex", gap: 10, pointerEvents: "none", background: "rgba(34,34,34,0.55)", padding: "3px 8px", borderRadius: 6 }}>
        <span>◆ site</span>
...
export default function App() {
  ...
  //
  // IMPORTANT: skip a poll tick if a local change was made very recently.
  // saveData() writes to Supabase asynchronously (not awaited from persist,
  // so the UI stays responsive) — without this guard, a poll landing in
  // that brief window would fetch data from BEFORE the write finished and
  // overwrite the local state, making a just-completed clock-in/report/etc.
  // appear to silently revert a few seconds after it happened.
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      if (Date.now() - lastPersistRef.current < 8000) return;
      try {
  ...
  const persist = useCallback((next) => {
    ...
    saveData(next).catch((e) => {
      notify("Saved on this device, but syncing to the server failed: " + (e?.message || e) + ". Check your connection — this change may not appear elsewhere yet.", "error");
    });
  }, [notify]);
    ...
    <div style={{ minHeight:"100vh", background:`radial-gradient(1200px 500px at 20% -10%, #282828 0%, ${COLORS.navy} 55%, #141414 100%)`, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"Inter, sans-serif", padding:20 }}>
      <div style={{ width:"100%", maxWidth:430 }}>
        ...
        <Card style={{ padding:22 }}>
          <div style={{ display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:6,background:COLORS.canvas,padding:5,borderRadius:11,marginBottom:18 }}>
...
function SupHome({ data, sup, mySites, openVisit, goTo, onStartVisit }) {
  const actions = [
  ...
  const firstName = (sup?.name || "").split(" ")[0];
  return (
    <div style={{ position: "relative" }}>
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontFamily: "Space Grotesk, sans-serif", fontWeight: 700, fontSize: 20, color: COLORS.ink }}>Hi {firstName || "there"}!</div>
        <div style={{ fontSize: 12.5, color: COLORS.inkMute, marginTop: 2 }}>{greeting} — here's your day at a glance</div>
      </div>

      <Card style={{ padding: 16, marginBottom: 18, background: COLORS.navy, color: "#fff", border: "none" }}>
        <div style={{ fontSize: 11.5, color: "rgba(255,255,255,0.7)", textTransform: "uppercase", letterSpacing: 0.4 }}>
          {openVisit ? "Currently on duty" : "Not clocked in"}
        </div>
        <div style={{ fontFamily: "Space Grotesk, sans-serif", fontWeight: 700, fontSize: 16, marginTop: 4 }}>
          {openVisit ? (data.sites.find((s) => s.id === openVisit.siteId)?.name || "On site") : "Tap the button below to clock in"}
        </div>
      </Card>
      ...
      <button
        ...
        style={{
          ...
          boxShadow: "0 8px 20px rgba(159,18,17,0.45)",
          fontSize: 11,
          fontWeight: 700,
          display: "flex",
          flexDirection: "column",
      ...
      >
        <span style={{ fontSize: 18, lineHeight: 1 }}>{openVisit ? "⏻" : "⏱"}</span>
        <span style={{ fontSize: 9, marginTop: 2 }}>{openVisit ? "Out" : "In"}</span>
      </button>
    </div>
  );
}
...
function SupCheckIn({ data, persist, sup, siteId, openVisit, notify, onDone }) {
  ...
  const saveVisit = (checkout=false) => {
    if (!coords) { notify("Get your live location first — tap \"Get live location\" above.","error"); return; }
    const blockedByGeofence = outOfRange && !(checkout && overrideApproved);
    if (blockedByGeofence) {
      if (checkout) notify("You're outside the site geofence. Use \"Request permission to clock out from here\" below, or move back within range.","error");
      else notify("You are outside the allowed geofence for this site.","error");
      return;
    }
    if (!selfie) { notify(`Take a selfie before clocking ${checkout ? "out" : "in"} — tap "Take selfie" above.`,"error"); return; }
    if (checkout) {
  ...
  return <div>
    ...
    <Button variant="amber" onClick={()=>saveVisit(!!activeVisit)} style={{width:"100%"}}>
...
function SupDailyReport({ data, persist, sup, mySites, notify, goHome }) {
  ...
  const rowIssue = (r) => {
    ...
    if (!r.fatherName.trim()) return "Father's name required";
    if (normalizePhone(r.contact).length !== 10) return "Contact must be 10 digits";
    if (!r.clientName.trim()) return "Client name required";
    return null;
  };
  ...
  const submit = () => {
    for (let i = 0; i < labourRows.length; i++) {
      ...
      const n = i + 1;
      if (!r.name.trim()) { notify(`Manpower entry ${n}: enter the labour's name.`, "error"); return; }
      if (!r.fatherName.trim()) { notify(`Manpower entry ${n}: enter the father's name.`, "error"); return; }
      if (normalizePhone(r.contact).length !== 10) { notify(`Manpower entry ${n}: contact number must be a valid 10-digit mobile number.`, "error"); return; }
      if (!r.clientName.trim()) { notify(`Manpower entry ${n}: enter the client name.`, "error"); return; }
    }
  ...
  return (
    <div>
      ...
      {labourRows.map((row) => {
        ...
        return (
        <Card key={row.key} style={{ padding: 12, marginBottom: 10, border: issue ? `1px solid ${COLORS.danger}` : undefined }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
            {issue ? <span style={{ fontSize: 11, color: COLORS.danger }}>{issue}</span> : <span />}
      ...
      <Button variant="amber" style={{ width: "100%" }} onClick={submit}>Submit daily report</Button>
    </div>
  );
}
  ...
  return <div><SectionTitle sub="Requirements uploaded by management">Current Requirements</SectionTitle>
    {rows.length===0?<Empty title="No current requirements"/>:<div style={{display:"flex",flexDirection:"column",gap:10}}>
      {rows.map(r=><Card key={r.id} style={{padding:14}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:8,marginTop:12,fontSize:11.5}}>
...
function SupOwnAttendance({data,sup}) {
  ...
  return <div><SectionTitle sub="Your clock-in history and working hours">My Attendance</SectionTitle>
    <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:10,marginBottom:14}}><KPI label="Sessions" value={rows.length}/><KPI label="Total hours" value={`${Math.floor(totalMs/3600000)}h ${Math.floor(totalMs%3600000/60000)}m`} tone={COLORS.info}/></div>
...
function AdminApp({ data, persist, session, onLogout, notify }) {
  ...
  const goToPage = (key) => { setPage(key); setSidebarOpen(false); };
    ...
    <div style={{ minHeight: "100vh", background: COLORS.canvas, fontFamily: "Inter, sans-serif", display: "flex" }}>
      ...
      {sidebarOpen && (
        <div
          ...
          className="pfs-sidebar-backdrop"
          style={{ display: "none", position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 199 }}
        />
      )}

      {/* sidebar */}
      <div className={"pfs-admin-sidebar" + (sidebarOpen ? " pfs-admin-sidebar-open" : "")} style={{ width: 224, background: COLORS.navy, color: "#fff", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ padding: "20px 18px", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <button onClick={() => setSidebarOpen(false)} className="pfs-sidebar-close" style={{ marginLeft: "auto", display: "none", background: "none", border: "none", color: "#fff", fontSize: 20, cursor: "pointer" }}>✕</button>
        </div>
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        <div className="pfs-admin-topbar" style={{ background: "#fff", borderBottom: `1px solid ${COLORS.line}`, padding: "12px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "relative", gap: 10 }}>
          <button onClick={() => setSidebarOpen(true)} className="pfs-sidebar-hamburger" style={{ display: "none", background: "none", border: `1px solid ${COLORS.line}`, borderRadius: 8, padding: "7px 10px", fontSize: 16, cursor: "pointer", flexShrink: 0 }}>☰</button>
...
function AdminDashboard({ data }) {
  ...
  return (
    <div>
      ...
      <div className="pfs-kpi-5" style={{ display: "grid", gridTemplateColumns: "repeat(5, minmax(0,1fr))", gap: 12, marginBottom: 20 }}>
      ...
      <div className="pfs-kpi-5" style={{ display: "grid", gridTemplateColumns: "repeat(5, minmax(0,1fr))", gap: 12, marginBottom: 22 }}>
      ...
      <div className="pfs-2col-stack" style={{ display: "grid", gridTemplateColumns: "minmax(0,1.4fr) minmax(0,1fr)", gap: 16, marginBottom: 20 }}>
      ...
      <div className="pfs-2col-stack" style={{ display: "grid", gridTemplateColumns: "minmax(0,1.4fr) minmax(0,1fr)", gap: 16 }}>
        <Card style={{ padding: 16 }}>
...
function EditSupervisorAssignment({ data, persist, notify, sup, onDeleted }) {
  ...
  const remove = async () => {
    ...
    try { await deleteRemoteRecord("supervisors", sup.id); } catch (e) { notify("Removed locally, but the backend delete failed: " + (e.message || e), "error"); }
...
function EditSiteForm({ data, persist, notify, site }) {
  ...
  const remove = async () => {
    ...
    try { await deleteRemoteRecord("sites", site.id); } catch (e) { notify("Removed locally, but the backend delete failed: " + (e.message || e), "error"); }
    notify("Site deleted.");
  };
  ...
  return (
    <div>
      ...
      <Card style={{ padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0,2fr) minmax(0,1.4fr) minmax(0,1fr) minmax(0,1fr) auto", gap: 8 }}>
