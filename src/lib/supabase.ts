...
export async function createAuthAccount(email: string, password: string, metadata: Record<string, any> = {}) {
  ...
  const body = await res.json();
  // Supabase returns an explicit "already registered" style error here when
  // auto-confirm is on and the email truly exists — surface that verbatim.
  ...
  const identityCount = Array.isArray(body.identities) ? body.identities.length : null;

  // A 200 OK with a populated session/user means it worked, full stop.
  if (hasSession || identityCount === null || identityCount > 0) {
    return { confirmed: hasSession };
  }

  // identityCount === 0 with no session is genuinely ambiguous from the
  // client alone: Supabase returns this shape BOTH when the email already
  // exists (anti-enumeration protection) AND in some project configs for
  // other reasons. Rather than guess, say so plainly and point at where to
  // check directly.
  throw new Error(
    `Supabase didn't create a usable session for ${email}. This usually means either (a) an account for this email already exists — check Authentication → Users in Supabase for it, or (b) "Confirm email" is still ON (Authentication → Providers → Email) and needs to be switched off.`
  );
}
...
function ownerKeyForRecord(entity: string, record: any, data: any) {
  ...
  // Most entities record the acting supervisor as `supervisorId`, but the
  // `attendance` entity (marking an employee present/absent) uses
  // `markedBy` instead — without this fallback, every attendance write
  // from a supervisor got owner_key=null and was silently rejected by RLS.
  const supervisorId = record.supervisorId || record.markedBy || null;
...
export async function deleteRemoteRecord(entity: string, recordId: string) {
  if (!supabaseConfigured) return;
  await rest(`pfs_records?entity=eq.${encodeURIComponent(entity)}&record_id=eq.${encodeURIComponent(recordId)}`, {
    method: 'DELETE',
  });
}
